...
| 2026-05-22 | `embedding_engine.py` and `backend/services/embedding_service.py` support multi-key Voyage AI failover. Keys tried in order: `VOYAGE_API_KEY_1`, `VOYAGE_API_KEY_2`, `VOYAGE_API_KEY_3`, `VOYAGE_API_KEY` (legacy). Failover triggers on quota/rate/billing errors. `.env.example` documents multi-key setup. | Each Voyage account gets 50M free tokens; switch to next key when one is exhausted. | Harthik |
...
| 15 — Pre-launch checklist | 2026-05-19 | Harthik signed off all pre-launch sections (infrastructure through final sign-off). |
